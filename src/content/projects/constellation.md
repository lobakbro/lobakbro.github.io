...
**The learning state assessor.** Bayesian Knowledge Tracing gives a probabilistic estimate of mastery for each node. A correct answer doesn't necessarily mean mastery — could be a guess. An incorrect answer doesn't mean ignorance — could be a slip. BKT models both, maintaining a running posterior probability per skill. The system doesn't think in binary; it thinks in likelihoods.

**The instructor policy engine.** This is where the decisions happen. Given the MAP, the current learning state, and spaced repetition scheduling (FSRS models the forgetting curve — too soon is wasted review, too late is relearning from scratch), the engine decides *what* to teach, *when* to teach it, and *how*. First exposure gets a different treatment than review. A fading skill gets reinforced before the system pushes into new territory that depends on it. The goal is to keep the learner at the frontier — the boundary between what they know and what they're ready for — because that's where learning actually happens, especially when recall is a struggle.
